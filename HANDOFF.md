...
- **コード最終コミット = お手本音声のリアル化（間・抑揚・対話/語り2モード）＋声設定UIを評価の上へ移動**。**本番に反映済み**。
  - 中身: Fishに `s2.1-pro`/`temperature0.88`/`top_p0.85`/`prosody.speed0.92`（emotionは存在しないため不追加）。台本に `[pause]`/`[低い声で]` 等のタグと独自記法 `[[SILENCE:ms]]`（ffmpegで無音挿入）を埋め込み。お手本は「対話版（営業=本人声＋客=汎用声）／語り版（営業の語りのみ・客は間で表現）」の2モード。`#voicePanel` を `#output` の上へ移動し分析開始時から表示。
  - 仕様書: `docs/SPEC_voice_realism.md`。新規テスト `tests/unit/voice_realism.test.ts`（`npm test` 全51件通過）。
...
## 残りの製品タスク
0. **お手本音声リアル化＋2モードの「耳での最終確認」**（本番反映済み・未試聴）。本番にゆかたん本人のアカウントでログイン→Fish/Anthropicキー設定(BYOK)→声サンプルをアップ＋同意→「対話版」「語り版」をそれぞれ生成し、①クロージング質問・価格の後に沈黙が入るか ②語尾が下がって言い切れているか／一本調子でないか ③語り版で客の声が無く間で対話が想像できるか を確認。違和感あれば `prosody.speed` や `temperature`（src/voice.ts）を微調整。
